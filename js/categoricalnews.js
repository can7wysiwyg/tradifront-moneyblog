...
async function fetchCategoryArticles(catId) {
    ...
    try {
        const response = await fetch(`${API_URL}/public/articles-by-category/${catId}`, {
            method: 'GET',
            headers: {
...
function renderCategoryPage(data, container) {
    ...
    // Clear previous subcategory state
    Object.keys(subcategoryState).forEach(key => delete subcategoryState[key]);
